...
//  @Test
  public void testFSOperations() throws IOException {
    ...
    // delete (file)
    Path test2DirPath = new Path("test2");
    fs.delete(test2DirPath, false);
    // assertFalse(fs.exists(test2DirPath));
...
//  @Test
  ...
  private AmazonS3 failTwiceWrapper(final AmazonS3 s3, final AtomicInteger counter) {
    ...
    expect(amazonS3.putObject(anyObject(PutObjectRequest.class))).andAnswer(() -> {
      if (counter.getAndDecrement() <= 0) {
        return s3.putObject((PutObjectRequest) EasyMock.getCurrentArguments()[0]);
      } else {
        throw new IOException("injected exception");
      }
    });
    expect(amazonS3.listObjects(anyString(), anyString())).andDelegateTo(s3);
    expect(amazonS3.listObjects(anyObject(ListObjectsRequest.class))).andDelegateTo(s3);
    expect(amazonS3.getObject(anyObject(GetObjectRequest.class))).andDelegateTo(s3);
